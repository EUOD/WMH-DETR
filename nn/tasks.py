...
# from ultralytics.nn.extra_modules.hgblock import *
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_LWGA, C2f_CSSC, C2f_CNCM, C2f_HFRB, C2f_EVA, C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, HSG_X_HWFEBlock, HSG_X_TKSABlock, DualBranchFusionBlock, HWFEBlock, TKSABlock, C2f_SKATBlock, C2f_WaveletBlock):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            c2 = m.channel
                   ...
                   FSA, AttentiveLayer, EUCB_SC, Res_block
        ...
        # --------------ASF--------------
            ...
            c2 = c1[0]
            args = [c1]
        elif m in {HAFB, MFM, LCA, HFFE, DynamicFeatureFusion}:
            c1 = [ch[x] for x in f]
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
